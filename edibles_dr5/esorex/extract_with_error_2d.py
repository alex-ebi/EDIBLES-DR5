...
def parse_red_name(wave_map_file):
    if wave_map_file.name == 'wave_map_blue.fits':
        xfb_file = 'red_2d_science_blue.fits'
    elif wave_map_file.name == 'wave_map_redl.fits':
        xfb_file = 'red_2d_science_redl.fits'
    elif wave_map_file.name == 'wave_map_redu.fits':
        xfb_file = 'red_2d_science_redu.fits'
    else:
...
def main():
    ...
    output_dir_online = Path('/home/alex/diss_dibs/edibles_reduction/extracted_added_2d')
    cleanup = False
    ...
    for _, row in obs_list.iterrows():  # Iterate through all OB's
        ...
        for sub_dir in sub_dirs:
            ...
            print(wave_maps[0])
            red_file = sub_dir / parse_red_name(wave_maps[0])
            sof_file = sub_dir / 'input.sof'
            modify_sof(sof_file, wave_maps[0], red_file, row['MJD-OBS'])
            ...
            #         ' --reduce.extract.method="2d" '
            #         f'{sub_dir / "input_edibles.sof"}')

            red_files = list(sub_dir.glob('*red_2d_science_*'))
            print(red_files)
            # Extract reductions which were made with super flats
            for red_file in red_files:
                red_name = red_file.name
                red_err_file = sub_dir / red_name.replace('red_', 'error_')

                # Read wavelength map
                print('Reading wave map', wm_file)

                # Open 2D file. if none is found, skip it.
                with fits.open(red_file) as f:
                    red_hdr = f[0].header
                    red_data = f[0].data

                # Open 2D error file
                with fits.open(red_err_file) as f:
                    err_red = f[0].data
        ...
        for file_name in file_set:
            ...
            flux_cols = []
            err_cols = []
            for iter_name, red_data, err_red, red_hdr in spec_list:
                if iter_name == file_name:
                    my_hdr = red_hdr
                    flux_cols.append(red_data)
                    err_cols.append(err_red ** 2)
            ...
            # plt.imshow(add_flux, aspect='auto', norm='log')
            # plt.show()

            # Save file
            # Write data to file
            hdu_flux = fits.ImageHDU(data=add_flux)
            hdu_flux_err = fits.ImageHDU(data=add_error)



            primary_hdu = fits.PrimaryHDU(header=my_hdr)

            hdul = fits.HDUList(hdus=[primary_hdu, hdu_flux, hdu_flux_err])
